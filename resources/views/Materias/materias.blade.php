            ...
            <div class="row">
                <div class="table-responsive">
                  <table class="table table-bordered table-bordered dataTable"  width="100%" cellspacing="0">
                    ...
                    @auth
                    @if (auth()->user()->role == 'administrador')
                    <form action="{{ route('materias.destroy', $row->id_mater) }}" method="post">
                    <a href="{{ route('materias.show', $row->id_mater) }}" class="btn btn-primary btn-sm">Ver</a>
                    <a href="{{ route('materias.edit', $row->id_mater) }}" class="btn btn-warning btn-sm">Editar</a>
                    @csrf
                    @method('DELETE')
